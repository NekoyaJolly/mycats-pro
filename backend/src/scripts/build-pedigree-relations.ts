...
async function buildPedigreeRelations() {
  ...
  try {
    ...
    relatedPedigrees.forEach(p => {
      const father = p.fatherPedigree ? `父: ${p.fatherPedigree.pedigreeId} (${p.fatherPedigree.catName})` : '';
      const mother = p.motherPedigree ? `母: ${p.motherPedigree.pedigreeId} (${p.motherPedigree.catName})` : '';
